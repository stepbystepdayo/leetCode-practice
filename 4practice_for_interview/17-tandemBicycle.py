'''
  ...
  dictates the speed of the bicycle. So if person A pedals at a speed of 5, and person B pedals at a speed of 4, the tandem bicycle moves at a speed of 5
  ...
  input parameter, fastest. if fastest = true, your function should rreturn the maximum possible total speed; otherwise it should return the minimum total speed.
  ...
  blue-shirt riders) who have speeds of 1,3,4,5, and id they're paired on tandem bicycles as followa [1,4],[5,3], then the total speed of these tandem bicycles is 4 +5 = 9 
'''

def solution(red, blue, fastest):
  red.sort()
  blue.sort()

  if not fastest:
    reverseArrayInPlace(red)
  
  totalSpeed = 0
  for index in range(len(red)):
    rider1 = red[index]
    rider2 = blue[len(blue) - index - 1]
